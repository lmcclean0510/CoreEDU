// src/app/admin/components/JsonImportDialog.tsx
...
export function JsonImportDialog({ 
  ...
  const handleImport = async () => {
    try {
      ...
      if (result.success > 0) {
        onImportComplete(); // Refresh the flashcards list
        if (result.failed === 0) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Import multiple flashcards at once using JSON format. Perfect for bulk content creation.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-hidden flex flex-col gap-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Paste your flashcards JSON array here..."
              rows={8}
            ...
            <div className="flex gap-2">
              ...
              {parseResult && (
                <Badge variant={parseResult.isValid ? "default" : "destructive"}>
                  {parseResult.isValid ? 
                    `✓ ${parseResult.validCount} valid flashcards` : 
                      ...
                      <div className="space-y-1">
                        ...
                        {validationErrors.map((error, index) => (
                          <div key={index} className="text-sm">
                            {error.index >= 0 ? `Item ${error.index + 1}` : 'JSON'}: {error.field} - {error.message}
                          </div>
                        ))}
                      </div>
                    </AlertDescription>
                  ...
                  <Card>
                    <CardHeader className="pb-3">
                      <CardTitle className="text-sm font-medium flex items-center gap-2">
                        ...
                        Preview ({previewData.length} flashcards ready to import)
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-2 max-h-40 overflow-y-auto">
                        {previewData.slice(0, 5).map((flashcard, index) => (
                          <div key={index} className="flex items-center justify-between p-2 bg-muted rounded text-sm">
                            <div>
                              <span className="font-medium">{flashcard.term}</span>
                              <span className="text-muted-foreground ml-2">({flashcard.subject})</span>
                            </div>
                        ...
                        {previewData.length > 5 && (
                          <div className="text-center text-sm text-muted-foreground">
                            +{previewData.length - 5} more flashcards...
                          </div>
                        )}
                      </div>
                    </CardContent>
        ...
        <div className="flex justify-between">
          ...
          >
            ...
            {isImporting ? 'Importing...' : `Import ${previewData.length} Flashcards`}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
